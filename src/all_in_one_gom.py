...
class ImageGraphPreprocessor:
    ...
    def __init__(self, config: dict):
        ...
        # ---- profondità ----------------------------------------------------------
        self.depth_relation_threshold = config.get("depth_relation_threshold", 0.10)
        ...
        # -------------------------------- relazione 2‑D & profondità -----------------
        self.angle_tolerance      = config.get("angle_tolerance_deg", 20)   # ± deg
        self.iou_min_for_depthrel = config.get("iou_min_for_depthrel", 0.05)
    ...
    @torch.inference_mode()
    def _relative_depth(self, image_pil, centres, masks=None):
        img_np = cv2.cvtColor(np.array(image_pil), cv2.COLOR_RGB2BGR)
        depth  = self.midas(self.midas_trans(img_np).to(self.device)
                            ).squeeze().cpu().numpy()
        H_d, W_d = depth.shape
        W, H     = image_pil.size
        ...
        for k, (cx, cy) in enumerate(centres):
            if masks is not None and k < len(masks):
                ys, xs = np.where(masks[k]["segmentation"])
                if len(xs) > 30:                    # area minima
                    # ridimensiona le coordinate della maschera nel sistema MiDaS
                    xs = (xs * W_d / H).astype(int)   # attenzione a H/W!
                    ys = (ys * H_d / W).astype(int)
                    vals.append(np.median(depth[ys, xs]))
                    continue        # salta il punto centrale
            # fallback: punto al centro
            x_d = int(np.clip(cx / W * W_d, 0, W_d - 1))
            y_d = int(np.clip(cy / H * H_d, 0, H_d - 1))
            vals.append(depth[y_d, x_d])

        vals = np.asarray(vals, dtype=float)
        rng  = np.ptp(vals) or 1.0
        return ((vals - vals.min()) / rng).tolist()
    ...
    def _infer_relationships_improved(
            self,
            image_pil: Image.Image,
            boxes: list,
        ...
        """
        Inference delle relazioni geometriche + CLIP + profondità.
        Restituisce una lista di dict con chiavi:
            src_idx, tgt_idx, relation, distance  (e clip_sim opzionale)
        """
        if labels is None:
        ...
        n        = len(boxes)
        centres  = [((b[0] + b[2]) / 2, (b[1] + b[3]) / 2) for b in boxes]
        depthval = self._relative_depth(image_pil, centres)

        rels: list[dict] = []

        # ------------------------------------------------------------------
        # 1) Relazioni 2‑D (above / below / left_of / right_of)
        # ------------------------------------------------------------------
        ang_tol = self.angle_tolerance        # (deg)
        for i in range(n):
            for j in range(n):
                if i == j:
                    continue
                dx, dy = centres[j][0] - centres[i][0], centres[j][1] - centres[i][1]
                dist   = math.hypot(dx, dy)
                if dist < self.min_distance or dist > self.max_distance:
                    continue

                ang = (math.degrees(math.atan2(dy, dx)) + 360) % 360  # 0° = →
                if 45 - ang_tol <= ang <= 135 + ang_tol:
                    relation = "below"      # j è sotto i
                elif 225 - ang_tol <= ang <= 315 + ang_tol:
                    relation = "above"
                elif 135 - ang_tol <= ang <= 225 + ang_tol:
                    relation = "left_of"
                else:
                    relation = "right_of"
        ...
        # ------------------------------------------------------------------
        # 2) CLIP template (inside / holding / …)
        # ------------------------------------------------------------------
        REL_TEMPLATES = [
            "on top of", "under", "inside", "holding", "riding",
            "touching", "next to", "in front of", "behind"
        ]
        for i, box_i in enumerate(boxes):
            for j, box_j in enumerate(boxes):
                ...
                rel, sim = self._clip_relation(union, lab_i, lab_j, REL_TEMPLATES)
                if sim > 0.23:
                    dist = math.hypot(centres[j][0] - centres[i][0],
                                      centres[j][1] - centres[i][1])
                    rels.append({"src_idx": i, "tgt_idx": j,
                                "relation": rel.split()[1],  # “on”, “under”, …
                                "clip_sim": sim, "distance": dist})

        # ------------------------------------------------------------------
        # 3) Profondità (front_of / behind)
        # ------------------------------------------------------------------
        # soglia adattiva: massimo fra fisso e 0.15*IQR
        iqr  = np.percentile(np.abs(depthval - np.median(depthval)), 75)
        thr  = max(self.depth_relation_threshold, 0.15 * iqr)

        for i in range(n):
            for j in range(n):
                if i == j:
                    continue
                delta = depthval[j] - depthval[i]          # >0 --> j più vicino
                if abs(delta) < thr:
                    continue
                # richiediamo sovrapposizione minima in XY
                if self._compute_iou(boxes[i], boxes[j]) < self.iou_min_for_depthrel:
                    continue
                rels.append({"src_idx": i, "tgt_idx": j,
                            "relation": "front_of" if delta < 0 else "behind",
                            "distance": abs(delta)})

        # ------------------------------------------------------------------
        # 4) ConceptNet (PartOf, UsedFor…)
        # ------------------------------------------------------------------
        for i in range(n):
            for j in range(n):
                if i == j:
        ...
        # dedup → una sola relazione (la più “vicina”) per coppia
        return self._unify_pair_relations(rels)
    ...
    def _limit_relationships_per_object(self, relationships: list, boxes: list) -> list:
        ...
        for src_idx, rlist in rels_by_src.items():
            sorted_rlist = sorted(rlist, key=lambda r: r.get("distance", 1e9))
    ...
    def process_single_image(
        ...
        rels_all = self._unify_pair_relations(
            self._infer_relationships_improved(image_pil, boxes, labels)   # <-- passate l’immagine
        )
...
def parse_preproc_args():
        ...
        parser.add_argument("--depth_relation_threshold",
                            type=float, default=0.10,
                            help="Soglia |Δdepth| per etichettare front_of / behind")
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "depth_relation_threshold": args.depth_relation_threshold
    }

        # ✅ AGGIUNGI IL BLOCCO DI CLEAR CACHE QUI
    if args.clear_cache:
